...
Near-verbatim port of classifier_training/launcher.py's SLURM-script-generation / dry-run /
debug_split / sequential / config-name machinery, repointed at deepspot_training.experiment.
No patch-grid ("grid_experiment") mode here — not requested for the DeepCell pipeline.
...
Usage:
    ...
    # Run every split sequentially in a single SLURM job on one GPU
    python -m src.python.deepspot_training.launcher \
        data.model_name=virchow_v2 data.correction_name=scanorama slurm.sequential=true

    # Start from a different top-level config file (must live under configs/)
    python -m src.python.deepspot_training.launcher \
...
def _path_relevant_data_overrides(cfg: DictConfig) -> list[str]:
    """CLI overrides for every data.* field that _split_results_summary_path's output
    path depends on. Must be forwarded into every generated SLURM script -- otherwise a
    launcher-level override (e.g. data.n_neighbors=0) changes where *this process* looks
    for results_summary.yaml without changing where the submitted job actually writes
    them (the subprocess only ever inherits data.model_name/data.correction_name plus
    whatever config-name's file has baked in), silently desyncing the skip-check from
    reality.
    """
    ...
    return [
        ...
        f"data.num_genes={int(cfg.data.num_genes)}",
        f"data.n_neighbors={int(cfg.data.n_neighbors)}",
    ]


def _split_results_summary_path(split_idx: int, cfg: DictConfig, repo_root: Path) -> Path:
    outputs_folder_name = str(OmegaConf.select(cfg, "output_dir", default="outputs_deepspot"))
...
def _generate_slurm_script(
    ...
    if cfg.slurm.partition == "gpu":
        return textwrap.dedent(f"""\
            #!/bin/bash
            #SBATCH --job-name=deepcell_{split_label}
            ...
            python -m src.python.deepspot_training.experiment {overrides}
        """)
    else:
        return textwrap.dedent(f"""\
            #!/bin/bash
...
def _generate_slurm_script_sequential(
    ...
    for split_idx in split_indices:
        ...
        run_lines.append(f"python -m src.python.deepspot_training.experiment {overrides}")
    runs_block = "\n".join(run_lines)
    # See classifier_training/launcher.py's identical helper for why this re-indent step is
    # needed before textwrap.dedent on the outer template.
    runs_block_indented = textwrap.indent(runs_block, " " * 12).lstrip(" ")

    if cfg.slurm.partition == "gpu":
        return textwrap.dedent(f"""\
            #!/bin/bash
            #SBATCH --job-name=deepcell_sequential
            #SBATCH --partition={cfg.slurm.partition}
            #SBATCH --gres={cfg.slurm.gres}
            #SBATCH --mem={cfg.slurm.mem}
            #SBATCH --time={cfg.slurm.time}
...
def main(cfg: DictConfig) -> None:
    ...
    # The generated SLURM scripts invoke experiment.py as a fresh process -- it must be told which
    # config this launcher itself was invoked with (its @hydra.main default is 'deepspot_train'),
    # otherwise it silently falls back to configs/deepspot_train.yaml regardless of --config-name
    # passed here.
    config_name: str = HydraConfig.get().job.config_name or "deepspot_train"
    log_dir = (
        repo_root
